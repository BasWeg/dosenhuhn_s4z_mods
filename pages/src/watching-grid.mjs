...
common.settingsStore.setDefault({
    ...
    },
    {
        id: 'default-screen-2',
        sections: [{
            type: 'single-data-field',
            id: 'default-single-data-fields',
            groups: [{
                id: 'default-power',
                type: 'power',
            }],
...
const groupSpecs = {
    ...
    hr: {
        title: 'Heart Rate',
        backgroundImage: 'url(/pages/images/fa/heartbeat-duotone.svg)',
        fields: [{
            id: 'hr-cur',
    ...
    cadence: {
        title: 'Cadence',
        backgroundImage: 'url(/pages/images/fa/solar-system-duotone.svg)',
        fields: [{
            id: 'cad-cur',
    ...
    draft: {
        title: 'Draft',
        backgroundImage: 'url(/pages/images/fa/wind-duotone.svg)',
        fields: [{
            id: 'draft-cur',
    ...
    event: {
        ...
        backgroundImage: 'url(/pages/images/fa/flag-checkered-duotone.svg)',
        fields: [{
            id: 'ev-place',
    ...
    pace: {
        title: speedLabel,
        backgroundImage: 'url(/pages/images/fa/tachometer-duotone.svg)',
        fields: [{
            id: 'pace-cur',
...
async function getTpl(name) {
    return await sauce.template.getTemplate(`/pages/templates/${name}.html.tpl`);
}
...
async function createLineChart(el, sectionId, settings) {
    ...
    const charts = await import('/pages/src/charts.mjs');
...
export async function main() {
    ...
    grid.on('resizestop', function(event, items) {
        ...
        const width = ((window.innerWidth / 12) * items.getAttribute("gs-w"))/16;
        //items.setAttribute("style","font-size: 16px");
        items.style.fontSize = width + 'px';
        console.log(width);
        resizeCharts();
    });    
    ...
    for (const [sIndex, screen] of settings.screens.entries()) {
       ...
       // console.log(screenEl);
        const newelem = grid.addWidget({w: 2,h:2, content:screenEl.outerHTML});
        //content.appendChild(screenEl);
        const renderer = new common.Renderer(newelem, {
            id: screen.id,
            fps: null,
        ...
        for (const section of screen.sections) {
            ...
            const sectionEl = newelem.querySelector(`[data-section-id="${section.id}"]`);
